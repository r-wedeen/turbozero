...
def make_sampled_nn_eval_fn(
    ...
    def eval_fn(state, params, move_length=1, temperature=1.0, env=None):
        # get the policy and value from the neural network
        policy_logits, value = nn.apply(params, state_to_nn_input_fn(state)[None,...], train=False, move_length=move_length, temperature=temperature, env=env)
        # apply softmax to the policy logits
        return jax.nn.softmax(policy_logits, axis=-1).squeeze(0), value.squeeze()
